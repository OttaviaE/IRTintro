...
institute: "Departmente of Psychology and Cognitive Science, UniTN"
...
- When the parameter of the person matches the parameter of the item and there are no lucky guesses and no careless errors, then the probability of observing a correct response is 50% 
...
plot(theta1, IRT(theta1, a = 200, b = 0), 
     ...
     ylim = c(0,1 ), cex.lab= 2, 
     cex.axis =1.5, xlab = expression(theta), lwd = 3, col = "firebrick")
lines(theta1, IRT(theta1, a = 20, b = 0), 
...
lines(theta1, IRT(theta1, a = 20, b = 0), 
     type = "l", lwd = 2, lty = 2, col = "firebrick")
lines(theta1, IRT(theta1, a = 4, b = 0), 
     type = "l", lwd = 2, lty = 2, col = "royalblue")
...
abline(h = .50, lty = 3, col = "gray78", lwd = 2)
text(x= 3.2, 
     y = 0.90, "item2", 
     col = my_colors[5], cex = 2)
abline(v = b[1], lty = 2, col = "red", lwd = 2)
abline(v = b[2], lty = 2, col = "red", lwd = 2)
abline(h = g[1] + (1-g[1])/2, lty = 3, col = my_colors[1], lwd = 2)
abline(h = g[2] + (1-g[2])/2, lty = 3, col = my_colors[5], lwd = 2)
...
lines(theta, IRT(theta, a = a[2], b = b[2], c= g[2]), 
     col = my_colors[5], lwd = 3)
lines(theta, IRT(theta, a = a[2], b = b[2], c= 0), 
     col = "gray", lwd = 3)
lines(theta, IRT(theta, a = a[1], b = b[1], c= 0), 
     col = "gray", lwd = 3)
...
c1 = item_info(b,a,  guessing = F)
Theta <- matrix(seq(-4,4, length.out=1000))
check = data.frame(theta = Theta)
check1 = data.frame(theta = Theta)
temp = NULL
temp1 = NULL
for (i in 1:length(c)) {
  temp = cbind( temp, c[[i]])
  temp1 = cbind( temp1, c1[[i]])
}

check = cbind(check, temp)
colnames(check)[2:ncol(check)] = paste0("item_info", 1:(ncol(check)-1))

check1 = cbind(check1, temp1)
colnames(check1)[2:ncol(check1)] = paste0("item_info", 1:(ncol(check1)-1))
check1$item_info2 = unlist(item_info(b[2], a[2], guessing = F))
d <- do.call('cbind',c)
sum_info <- rowSums(d)
d1 <- do.call('cbind',c1)
...
plot(Theta, check[, 1], ylim= c(0, 1), cex.lab= 2, 
     ...
     type = "l", lwd =2, 
     col = my_colors[1])
lines(theta, check1[, 1], 
      lty = 3, lwd=3, col = "gray")
text(x= 0, y = 0.4, colnames(si)[1], col = my_colors[1], cex = 2)

for ( i in 2:length(b)) {
  lines(theta, check[, i], 
      lty = 1, lwd=3, col = my_colors[i])
   lines(theta, check1[, i], 
      lty = 3, lwd=3, col = "gray")
}
...
```{r}
sum_info1 <- rowSums(check1)
plot(theta, sum_info, cex.lab= 2, 
     cex.axis =1.5,
       xlab = expression(theta), ylab = "Information", 
     type = "l", lwd =2, 
     col = "black", ylim = c(0,1))
lines(theta, sum_info1, lty = 3, lwd = 2, col = "gray")
for ( i in 1:length(b)) {
  lines(theta, check[, i], 
      lty = 1, lwd=3, col = my_colors[i])
}
...
plot(theta, sqrt(1/sum_info), cex.lab= 2, 
     ...
     type = "l", lwd =2, 
     col = "firebrick")
lines(theta, sqrt(1/sum_info1), col ="gray", lty = 3, lwd = 2)
...
text(x= 3.2, 
     y = 0.80, "item2", 
     col = my_colors[5], cex = 2)
lines(theta, IRT(theta, a = a[2], b = b[2]), 
     col = "gray", lwd = 2, lty = 2)
lines(theta, IRT(theta, a = a[1], b = b[]), 
     col = "gray", lwd = 2, lty = 2)
```
...
\footnotesize
```{r, results='asis'}
items = data.frame(tired_q$e[1], 
                   tired_q$e[2],
                   tired_q$e[3])
print(xtable(items), comment = FALSE, include.rownames=T)
```
