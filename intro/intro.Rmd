...
subtitle: "Introduction to IRT models"
...
Giorgio's behaviors (\color{manifeste}**observed variables**\normalcolor) can be explained by considering the latent variable ***\color{latenti}extraversion*** 
...
- *Local independence*: The correlation between the \color{manifeste}observed variables \normalcolor after controlling for the influence of the \color{latenti}latent variable\normalcolor
...
- Four-Parameter Logistic Model (4-PL)
...
- When the parameter of the person matches the parameter of the item, then the probability of observing a correct response is 50% (Only for the 1-PL)
...
The probability that person $p$ responds correctly to item $i$ is formalized ad


$$P(x_{pi} = 1| \theta_p, b_i) = \dfrac{\exp(\theta_p - b_i)}{1 + \exp(\theta_p - b_i)}$$


$\theta_p$: Ability of the person (i.e., latent trait level of the person) $\rightarrow$ The value of $\theta_p$, the higher the amount of latent trait of $p$
...
### Item with different locations: ICC
...
$Q = 1- P_i(\theta_p, b_i)$ is the probability of choosing the incorrect response

###

The IIFis maximized when $\theta_p = b_i$ $\rightarrow$ $P(x_{pi}=1) = P(x_{pi}=0) =0.50$ $\rightarrow$ $I_i = .25$
...
The item is mostly informative for subjects with a latent trait level close to the location of the item $\rightarrow$ the higher the distance between the latent trait level of the person and the location of the item, the lower the IFF
...
### Items with difference locations: IIF
...
plot(Theta, check[, 1], ylim= c(0, 1), cex.lab= 2, 
     cex.axis =1.5, cex.main =3,
       xlab = expression(theta), ylab = "Information", 
     type = "l", lwd =2, 
     col = my_colors[1], main = "IIF- Item with different locations")
segments(-7, 0.25, 
         b[1], 0.25, 
         col = "red", lty = 3, lwd = 3)
segments(b[1], -0.25, 
...
$b_i$; difficulty of item $i$ (location of the item on the latent trait) $\rightarrow$
...
plot(theta, IRT(theta, a = -0.5, b = 0), 
     type = "l", cex.main = 3, 
     ylab = "P(x = 1)", ylim = c(0,1 ), cex.lab= 2, 
     cex.axis =1.5, xlab = expression(theta), lwd = 3, main = "Negative discrimination")
text(x= 3.2, 
     y = 0.95, expression(paste(a[1], "= -0.50")), 
     col = "black", cex = 2)
...
$Q_i = 1 - P_i(\theta, b_i, a_i)$ is the probability of giving the incorrect response
...
Item 1: $b_1 = 0$, $a_1 = 1.4$ e $c_1 = 0.2$, Item 2: $b_1 = 0$, $a_1 = 1.4$ e $c_1 = 0.3$ (item 2)
...
The probability of responding correctly is approximately $c$ (0.20, 0.30) for low levels of the latent trait
...
If there $k$ response options that are all equally plausible, then $c \cong \dfrac{1}{k}$
...
The lower then value of $d_i$, the lower the probability that a person with high level of the latent trait gives the correct response to item $i$
...
- Constraining the $a_i$ parameters of all items to be 1 $\rightarrow$ from 2-PL to 1_PL
...
The model that fit better the data is chosen
